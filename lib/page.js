...
_.assign(module.exports.prototype, {
    ...
    analyzeHtmlDepsAndAsync: function() {
        ...
        // console.log('>>>', file.subpath, requires, asyncs, loadUrls);
        ...
        // 
        // all logic of loadUrl added by lqlongli on 2015/11/25
        loadUrls.forEach(function(fileId) {
            if (HTTP_REG.test(fileId)) return;
            if (!pageDepMap[fileId]) {
                pageDepMap[fileId] = self.calFileDepsFromId(fileId, pageRes);
                pageDepMap[fileId].isLoadUrl = true;
            }
    ...
    injectJs: function(content, pageRes) {
        ...
        if (conf.autoPack) {
            ...
            Object.keys(pkg).forEach(function(pId) {
                ...
                if (!pFile.isLoadUrl && !pFile.isAsync && pFile.isJsLike) {
    ...
    generatePackageFile: function(depMap) {
        ...
        if (conf.jsAllInOne) {
            ...
            allInOneFile.setContent(allSyncContent);
    ...
    getMainJs: function() {
        ...
        // console.log('getMainJs:', this.file.subpath, this.file.extras.loadUrls, this.file.requires, this.file.asyncs, self.conf.libDict);
        ...
        // console.log('hasGotMainJs:', self.mainScriptId);
    ...
    injectResourceMap: function(content, resourceMap) {
        ...
        if (conf.outputResourceMapToMainJs) {
            ...
            if (this.mainScriptId) {
                ...
                // console.log('subpath', subpath, !!this.ret.pkg['/'+subpath]);
