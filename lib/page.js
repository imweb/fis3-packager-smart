...
_.assign(module.exports.prototype, {
    init: function() {
        ...
        if (conf.autoPack) {
            // console.log(Object.keys(analysis.depMap));
    ...
    injectAlisa: function(content, requires) {
        ...
        //     content = content.replace(conf.scriptPlaceHolder, script + LINE_BREAK + '$&');
        // } else {
    ...
    generatePackageFile: function(depMap) {
        ...
        Object.keys(depMap).forEach(function(id) {
            ...
            if (Page.combineCache[id]) { // 去重
                ...
                self.pkg[pkgFile.subpath] = pkgFile;
                return;
            }
            ...
            // Page.jsSourceHtmlDict[pkgFile.subpath] = [self.file.subpath];
