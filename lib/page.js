...
_.assign(module.exports.prototype, {
    init: function () {
        ...
        if (file.useCache) {
            ...
            if (file.cache) {
                // 需要监听的文件，做一次去重
                var watchFiles = {};
                [analysis.pageRes, analysis.cssDeps, analysis.asyncDeps, analysis.depMap].forEach(function (files) {
                    Object.keys(files).forEach(function (key) {
                        if (!watchFiles[key]) {
                            watchFiles[key] = files[key];
                        }
                    });
                });
                Object.keys(watchFiles).forEach(function (key) {
                    var watch = watchFiles[key];
                    if (watch.cache) {
                        file.cache.mergeDeps(watch.cache);
                    }
                    if (watch.realpath) {
                        file.cache.addDeps(watch.realpath);
                    }
                })
            }
        }
    ...
    processLoadUrl: function (content, depMap) {
        ...
        content = content.replace(reg, function (m, value, arg) {
            ...
            res.forEach(function (o) {
                if (o.arg) {
                    ret.push('var ' + o.arg + '=require("' + o.id + '");');
                }
            });
            return ret.join('');
        });
    ...
    generatePackageFile: function (depMap) {
        ...
        Object.keys(depMap).forEach(function (id) {
            ...
            if (self.conf.libDict && self.conf.libDict[id] || self.isIdInLoadUrl(id)) {
    ...
    generateSourceMap: function (analysis) {
        ...
        Object.keys(analysis.depMap).forEach(function (p) {
            ...
            // bugfix:
            // 如果有两个页面，一个页面用loadUrl，另一个不用loadUrl
            // 对于公共文件也就是 lib 参数指定的文件来说
            // 会出现不统一的情况，这里需要对公共文件的处理进行统一
            // 由于 md5 的原因，需要按照 loadUrl 的处理方式来走
            // 因为 loadUrl 的方式需要添加一些内容，再获取 md5
            if (self.conf.libDict && self.conf.libDict[p]) {
                var subpath = conf.output.replace('${id}', p),
                    _file = fis.file(fis.project.getProjectPath(), subpath);

                subpath = _file.subpath;
                var file2 = ret.pkg[subpath];
                file2.useHash = true;
                ret.map.pkg[combinedId] = {
                    uri: file2.getUrl(),
                    type: 'js'
